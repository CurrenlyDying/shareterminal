...
if ! command -v tmux >/dev/null 2>&1; then
  ...
  if command -v sudo >/dev/null 2>&1; then
    read -r -p "Attempt to install tmux using sudo and your package manager? [Y/n] " tmux_resp
    tmux_resp=${tmux_resp,,}
    if [[ $tmux_resp =~ ^(y|yes| )$ ]]; then
...
########################################
# Try to source shell configs (best effort)
########################################
info "Trying to source your shell configs so commands are available immediately (best effort)..."
